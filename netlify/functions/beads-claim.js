// beads-claim — proxy for POST /api/beads/claim/:id on the Railway Beads Service.
//
// POST /.netlify/functions/beads-claim
//   Body: { id: string }
//   Returns: { ok: true }
//
// Env vars required:
...
export default async (req) => {
  ...
  try {
    ...
    const res = await fetch(`${BEADS_SERVICE_URL}/api/beads/claim/${encodeURIComponent(id)}`, {
      method: 'POST',
      headers,
    ...
    if (!res.ok) {
      console.error(`[beads-claim] Railway error ${res.status}:`, data);
      return json({ error: data.error || 'upstream error' }, res.status);
    }

    return json(data);
  } catch (e) {
    console.error('[beads-claim] Railway call failed:', e.message);
